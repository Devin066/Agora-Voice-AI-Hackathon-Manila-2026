...
export default function PracticePage() {
  ...
  useEffect(() => {
    ...
    const { childAge } = JSON.parse(stored);
    const ageGroup = getAgeGroup(childAge);
    setPrompts(getPromptsForAge(ageGroup, 5));
  }, [router]);
  ...
  const handleNext = useCallback(() => {
    if (currentIndex + 1 >= prompts.length) {
      // Session done — save results
      const avgScore = (arr: number[]) =>
        Math.round(arr.reduce((a, b) => a + b, 0) / arr.length);

      const sessionResult = {
        scores: {
          pronunciation: avgScore(scores.map((s) => s.pronunciation)),
          fluency: avgScore(scores.map((s) => s.fluency)),
          articulation: avgScore(scores.map((s) => s.articulation)),
          confidence: avgScore(scores.map((s) => s.confidence)),
        },
        promptsCompleted: scores.length,
        totalPrompts: prompts.length,
        feedbackEvents: allFeedback,
      };
      localStorage.setItem("legacypp_session", JSON.stringify(sessionResult));
  ...
  }, [currentIndex, prompts.length, scores, allFeedback, router]);
  ...
  return (
    <main className="min-h-screen bg-bg flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-xl">
        ...
        <Card
          elevated
          className="text-center mb-6 py-10 border-2 border-primary/10"
        >
        ...
        {feedback && (
          <div
            className={cn(
              ...
              feedback.type === "tip" && "bg-accent/10 text-amber-700 border border-accent/30",
              feedback.type === "warning" && "bg-warning/10 text-warning border border-warning/30"
            )}
          >
