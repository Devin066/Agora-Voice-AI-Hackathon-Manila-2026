...
- `DEFAULT_TTS_ADDON` (default empty; optional if using addon-based TTS)
- `DEFAULT_TTS_VENDOR` (default empty; for vendor-based TTS fallback such as `microsoft`)
- `DEFAULT_TTS_PARAMS_JSON` (default empty; JSON object used as `properties.tts.params`)
- `DEFAULT_TTS_API_KEY` (default empty; Microsoft fallback key if `DEFAULT_TTS_VENDOR=microsoft`)
- `DEFAULT_TTS_REGION` (default empty; Microsoft fallback region)
- `DEFAULT_TTS_VOICE_ID` (default empty; Microsoft fallback voice id mapped to `voice_name`)
